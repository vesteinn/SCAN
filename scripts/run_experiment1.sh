...
#    python ../src/scan/train.py --device cuda --eval_interval 5000 --train ../data/SCAN/simple_split/tasks_train_simple.txt --valid ../data/SCAN/simple_split/tasks_test_simple.txt --model lstm --dropout 0 > $log_file
#    tail -n 1 $log_file
#    echo "--"
#done
...
for split in p1 p2 p4 p8 p16 p32 p64; do
    for i in {0..4}; do
        ...
        python ../src/scan/train.py --device cuda --eval_interval 5000 --train ${train_file} --valid ${valid_file} --model lstm > $log_file
        tail -n 1 $log_file
        echo "--"
    done
done
